	...
	<tr>
		...
		<td><select name="<portlet:namespace/>category"
			value='<c:out value="${requestScope.book.category}"/>'>
				<option value="java" <%= "java".equals(request.getAttribute("category")) ? "selected" : ""%>>Java</option>
				<option value=".net" <%= ".net".equals(request.getAttribute("category")) ? "selected" : ""%>>.NET</option>
				<option value="mobile" <%= "mobile".equals(request.getAttribute("category")) ? "selected" : ""%>>Mobile Technology</option>
				<option value="softwareEng" <%= "softwareEng".equals(request.getAttribute("category")) ? "selected" : ""%>>Software Engineering</option>
			</select>
			</td>
